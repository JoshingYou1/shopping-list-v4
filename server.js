...
app.put("/recipes/:id", jsonParser, (req, res) => {
  ...
  for (let i = 0; i < requiredFields.length; i++) {
    const field = requiredFields[i];
    if (!(field in req.body)) {
      const message = `Sorry, you're missing \`${field}\` in the request body`;
      console.error(message);
      return res.status(400).send(message);
    }
  }
  ...
  if (req.params.id !== req.body.id) {
    const message = ` Sorry, the request path id (${req.params.id}) does not match the request body id (${req.body.id})`
    console.error(message);
    return res.status(400).send(message);
  }

  console.log(`Updating \`${req.params.id}\` recipe`);
  Recipes.update({
    name: req.body.name,
    id: req.body.id,
